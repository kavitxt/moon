...
function cloneRec(node: unknown, seenNodes: object[] = []) {
  switch (typeof node) {
    ...
    case "symbol":
      //throw new Error
      log(`can't store a ${typeof node} in a moon storage!`, "error");
      return undefined;

    case "object":
      if (seenNodes?.includes(node)) throw new Error("can't store a circular reference in a moon storage!");
...
async function getDb(store: string) {
  ...
  const prom = openDB("moon", Date.now(), {
    upgrade(udb) {
...
export const signalOf = <T = any>(store: MoonStore<T>): (() => Record<string, T>) => store[symSig];
