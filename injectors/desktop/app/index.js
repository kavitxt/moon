...
const logger = new Proxy(console, {
  get: (target, key) =>
    function (...args) {
      return target[key].apply(console, ["[moon]", ...args]);
    },
});
...
// #region Bundle
const remoteUrl =
  process.env.MOON_BUNDLE_URL || "https://raw.githubusercontent.com/moon-inj/moon-builds/main/moon.js";
const localBundle = process.env.MOON_DIST_PATH;

const moonBundle = new Promise((resolve, reject) => {
  if (localBundle) {
    let data = fs.readFileSync(path.join(localBundle, "moon.js"), "utf8");
    data += `\n//# sourceMappingURL=file:////${path.join(localBundle, "moon.js.map")}`;
...
// #region IPC
electron.ipcMain.on("MOON_ORIGINAL_PRELOAD", (event) => {
  event.returnValue = event.sender.originalPreload;
});

electron.ipcMain.handle("MOON_BUNDLE_FETCH", () => moonBundle);
// #endregion
